...
class Task:
    ...
    def estimate_energy(self, task_type, duration_hours):
        """基于任务类型的精力需求，长任务加疲劳惩罚"""
        base = self.ENERGY_MAP.get(task_type, 0.6)
        # 超过2小时基准后，每多1小时精力需求+5%
        over_hours = max(0, duration_hours - 2.0)
        fatigue_penalty = 1.0 + 0.05 * min(over_hours, 4)
        return np.clip(base * fatigue_penalty, 0.2, 1.0)
